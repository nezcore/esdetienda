...
export default function RegisterPage() {
  ...
  const [generatingSlug, setGeneratingSlug] = useState(false) // Estado para generar slug disponible
  ...
  // Generar slug disponible automáticamente (verifica disponibilidad)
  const generateAvailableSlug = async (businessName: string): Promise<string | null> => {
  ...
  useEffect(() => {
    ...
    if (businessName.length >= 4) {
      ...
      if (generated.length >= 4) {
        ...
        if (!tenantSlug && !slugAutoGenerated && !generatingSlug) {
          console.log('✨ Primera vez sin slug - buscando uno disponible...')
          setGeneratingSlug(true)
          generateAvailableSlug(businessName).then(availableSlug => {
            if (availableSlug) {
              setTenantSlug(availableSlug)
              setSlugAutoGenerated(true)
              // Marcar inmediatamente como disponible ya que fue verificado
              setSlugStatus('available')
            }
            setGeneratingSlug(false)
          }).catch(error => {
            console.error('Error generando slug disponible:', error)
        ...
        else if (slugAutoGenerated && tenantSlug && tenantSlug !== generated && !generatingSlug) {
  ...
  useEffect(() => {
    if (currentStepIndex === 1 && businessName.length >= 4 && !tenantSlug && !slugManuallyCleared && !generatingSlug) {
      console.log('🎯 Navegaste al paso URL sin slug y no fue borrado manualmente - buscando uno disponible...')
      setGeneratingSlug(true)
      generateAvailableSlug(businessName).then(availableSlug => {
        if (availableSlug) {
          setTenantSlug(availableSlug)
  ...
  }, [currentStepIndex, businessName, tenantSlug, slugManuallyCleared, generatingSlug])
  ...
  const isSlugValid = slugPattern.test(tenantSlug) && tenantSlug.length >= 4 && slugStatus === 'available' && !generatingSlug
                      ...
                      <div className="space-y-2">
                        ...
                        <div className="flex">
                          ...
                          <div className="relative flex-1">
                            ...
                            />
                            <span className="absolute inset-y-0 right-3 flex items-center pointer-events-none">
                              {(slugStatus === 'checking' || generatingSlug) && (
                                <Loader2 className="h-5 w-5 text-brand-500 animate-spin" />
                              )}
                              {slugStatus === 'available' && tenantSlug && !generatingSlug && (
                                <CheckCircle className="h-5 w-5 text-emerald-500" />
                              )}
                              {(slugStatus === 'taken' || slugStatus === 'invalid') && tenantSlug && !generatingSlug && (
                                <XCircle className="h-5 w-5 text-red-500" />
                              )}
                            </span>
                          </div>
                        ...
                        {slugStatus === 'invalid' && tenantSlug && !generatingSlug && (
                          <p className="text-sm text-red-500 dark:text-red-400">
                            Usa al menos 4 caracteres en minúsculas, números o guiones.
                          </p>
                        )}
                        {slugStatus === 'taken' && !generatingSlug && (
